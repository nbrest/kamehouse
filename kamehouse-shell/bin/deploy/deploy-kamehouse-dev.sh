...
importKamehouse common/functions/kamehouse/deployment-functions.sh
...
deployKameHouseMobile() {
  ...
  if [[ "${MODULE}" == "kamehouse-mobile" ]]; then
    if [ -f "${KAMEHOUSE_ANDROID_APK_PATH}" ]; then
      ...
      ${HOME}/programs/kamehouse-shell/bin/mobile/kamehouse-mobile-upload-apk-to-device.sh -s
    else
