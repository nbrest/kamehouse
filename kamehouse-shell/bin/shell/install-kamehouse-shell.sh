...
# ./kamehouse-shell/bin/shell/install-kamehouse-shell.sh
...
updateUsername() {
  ...
  sed -i "s#USERHOME_LIN=\"/home/\${DEFAULT_KAMEHOUSE_USERNAME}\"#USERHOME_LIN=\"/home/${USERNAME}\"#g" "${KAMEHOUSE_SHELL_PATH}/bin/deploy/set-userhome.sh"
  sed -i "s#USERNAME=\"\${DEFAULT_KAMEHOUSE_USERNAME}\"#USERNAME=\"${USERNAME}\"#g" "${KAMEHOUSE_SHELL_PATH}/bin/sudoers/www-data/su.sh"
  
  sed -i "s#DEFAULT_KAMEHOUSE_USERNAME=\"\"#DEFAULT_KAMEHOUSE_USERNAME=\"${DEFAULT_KAMEHOUSE_USERNAME}\"#g" "${KAMEHOUSE_SHELL_PATH}/bin/functions/kamehouse/kamehouse-functions.sh"
...
updateLogLevel() {
  ...
  sed -i "s#LOG_LEVEL_NUMBER=3#LOG_LEVEL_NUMBER=${LEVEL_NUMBER}#g" "${KAMEHOUSE_SHELL_PATH}/bin/functions/log-functions.sh"
}
...
generateKameHouseShellPathFile() {
  ...
  local KAMEHOUSE_SHELL_SCRIPTS_PATH=`getPathWithSubdirectories "${HOME}/programs/kamehouse-shell/bin"`
  echo "KAMEHOUSE_SHELL_SCRIPTS_PATH=${KAMEHOUSE_SHELL_SCRIPTS_PATH}" >> ${KAMEHOUSE_SHELL_PATH_FILE}
}
...
getPathWithSubdirectories() {
  ...
  PATH_WITH_SUBDIRS=$(echo "$PATH_WITH_SUBDIRS" | grep -v /docker/docker-container)
  PATH_WITH_SUBDIRS=$(echo "$PATH_WITH_SUBDIRS" | grep -v /docker/release/java8-release/bin)
  PATH_WITH_SUBDIRS=$(echo "$PATH_WITH_SUBDIRS" | grep -v /docker/release/java8-release/docker)
