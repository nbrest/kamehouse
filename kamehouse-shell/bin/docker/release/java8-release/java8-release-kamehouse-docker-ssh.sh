...
importKamehouse common/functions/kamehouse/docker/release/java8-release-functions.sh
...
mainProcess() {
  if ${REMOVE_SERVER_KEY}; then
    ${HOME}/programs/kamehouse-shell/bin/docker/release/java8-release/java8-release-kamehouse-docker-server-key-remove.sh ${FIRST_RELEASE_FLAG}
  else
