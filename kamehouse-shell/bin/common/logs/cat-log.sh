...
catLogFunction() {
  if [ "${LOG_FILE_TO_CAT}" != "apache" ] \
    ...
    ${HOME}/programs/kamehouse-shell/bin/kamehouse/tail/tail-log.sh -f "${LOG_FILE_TO_CAT}" -q -n 2000
    ${HOME}/programs/kamehouse-shell/bin/kamehouse/tail/tail-log.sh -f "${LOG_FILE_TO_CAT}" -q -n 30
  fi
}
