# lin bashrc
source ${HOME}/programs/kamehouse-shell/bin/common/colors.sh
source ${HOME}/programs/kamehouse-shell/bin/common/bashrc/alias.sh
source ${HOME}/programs/kamehouse-shell/bin/common/bashrc/functions.sh
source ${HOME}/programs/kamehouse-shell/bin/common/bashrc/prompt.sh
source ${HOME}/programs/kamehouse-shell/bin/common/bashrc/message.sh
source ${HOME}/programs/kamehouse-shell/bin/lin/bashrc/path.sh
source ${HOME}/programs/kamehouse-shell/bin/lin/bashrc/alias.sh
source ${HOME}/programs/kamehouse-shell/bin/lin/bashrc/java-home.sh

export EDITOR=vim
