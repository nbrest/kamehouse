...
- Add `TOMCAT_TEXT_USER` and `TOMCAT_TEXT_PASS` env variables to `${HOME}/.kamehouse/config/.shell/shell.pwd` for groot to access tomcat manager's api. See [docker shell.pwd](/docker/keys/shell.pwd) as an example.
