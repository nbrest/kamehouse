...
public interface TestUtils<T, D> {
  ...
  /**
   * Verifies that all the attributes of the returned entity match the ones of the expected entity.
   */
  public void assertEqualsAllAttributes(T expectedEntity, T returnedEntity);

  /**
   * Verifies that the lists are identical, checking it's size, and for each element, checking that
