...
import com.nicobrest.baseapp.exception.BaseAppForbiddenException;
...
public class DragonBallController {
  ...
  public ResponseEntity<List<DragonBallUser>> getUsers(@RequestParam(value = "action",
    ...
    switch (action) {
      case "BaseAppNotFoundException":
        throw new BaseAppNotFoundException(
            "*** BaseAppNotFoundException in getUsers ***");
        // break;
  ...
  public ResponseEntity<String> getUsersByEmail(@PathVariable String email) {
     ...
     * e) { e.printStackTrace(); throw new BaseAppBadRequestException(
  ...
  public ResponseEntity<?> putUsersUsername(@PathVariable Long id,
    ...
    if (!id.equals(dragonBallUser.getId())) {
      throw new BaseAppForbiddenException("Id in path variable doesn´t match"
          + "id in request body.");
    }
  ...
  private String convertToJsonString(Object obj) {
    ...
    try {
      ...
      e.printStackTrace();
      throw new BaseAppServerErrorException("Error mapping Object to a Json string", e);
    }
    return jsonString;
  }
}
