...
function isAuthorizedUser($username, $password) {
  ...
  if (isLinuxHost()) {
     ...
     */
    $shellUsername = trim(shell_exec("\${HOME}/programs/kamehouse-shell/bin/kamehouse/get-username.sh"));
    $shellCommandOutput = shell_exec("sudo -u " . $shellUsername . " \${HOME}/programs/kamehouse-shell/bin/common/sudoers/www-data/exec-script.sh -s 'kamehouse/kamehouse-groot-login.sh' -a '" . $scriptArgs . "'");
  } else {
