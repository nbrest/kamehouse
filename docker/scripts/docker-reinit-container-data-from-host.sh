...
reinitMyScripts() {
  ...
  if [ "${DATA_SOURCE}" == "docker-init" ]; then
    ssh -p ${DOCKER_PORT_SSH} ${DOCKER_USERNAME}@localhost -C 'cp -v -f /home/nbrest/git/kamehouse/docker/keys/.cred /home/nbrest/my.scripts/.cred/'
  else
...
reinitMysql() {
  ...
  "docker-init"|"docker-backup"|"host-backup")
    ...
    ssh -p ${DOCKER_PORT_SSH} ${DOCKER_USERNAME}@localhost -C 'sudo mysql -v < /home/nbrest/git/kamehouse/kamehouse-shell/my.scripts/kamehouse/sql/mysql/setup-kamehouse.sql'
    ssh -p ${DOCKER_PORT_SSH} ${DOCKER_USERNAME}@localhost -C 'sudo mysql kameHouse < /home/nbrest/git/kamehouse/kamehouse-shell/my.scripts/kamehouse/sql/mysql/spring-session.sql'
